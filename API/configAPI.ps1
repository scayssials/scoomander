...
Function ApplyConfigurationFile([String]$configPath, [string[]]$include, [String[]]$exclude) {
    ...
    foreach ($installSpec in $scoopConf.install) {
        if ($installSpec -ne "" -and !($installSpec -like "#*")) {
            if ($installSpec -match '(?<type>[^:]+):(?:(?<bucket>[a-zA-Z0-9-]+)\/)?(?<name>.*.json$|[a-zA-Z0-9-_.]+)(?:@(?<version>.*))?') {
                ...
                if ((!$include -or $include.Contains($name)) -and (!$exclude -or !$exclude.Contains($name))) {
                    if ($type -eq "extra") {
...
Function UnapplyConfigurationFile([String]$configPath, [string[]]$include, [String[]]$exclude) {
    $scoopConf = (Get-Content "$configPath\conf.json") | ConvertFrom-Json
    $extrasPath = "$configPath\extras"
    # uninstall specs
